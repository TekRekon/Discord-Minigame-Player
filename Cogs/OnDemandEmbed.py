...
class OnDemandEmbed(commands.Cog):
    ...
    description = ''
    color = 0xff0000
    thumbnail = 'https://cdn.discordapp.com/attachments/549334435737239566/693915786665787583/Annotation_2020-03-29_153201.png'
    ...
    async def embed(self, ctx):
        if ctx.author != self.bot.user and ctx.message.channel != OnDemandEmbed.oneWordStoryChannel:
            OnDemandEmbed.thumbnail = 'https://cdn.discordapp.com/attachments/549334435737239566/693915786665787583/Annotation_2020-03-29_153201.png'
            ...
            try:
                def check(message):
                    return message.author == ctx.author and len(message.content) > 0 and message.channel == ctx.message.channel
                ...
                def generate_example_embed():
                    if OnDemandEmbed.title == 'None' and OnDemandEmbed.description == 'None':
                        ...
                        OnDemandEmbed.example_embed = discord.Embed(title=OnDemandEmbed.title, description=OnDemandEmbed.description, color=OnDemandEmbed.color)
                        OnDemandEmbed.user_embed = discord.Embed(title=OnDemandEmbed.title, description=OnDemandEmbed.description, color=OnDemandEmbed.color)
                    elif OnDemandEmbed.title == 'None':
                        OnDemandEmbed.example_embed = discord.Embed(description=OnDemandEmbed.description, color=OnDemandEmbed.color)
                        OnDemandEmbed.user_embed = discord.Embed(description=OnDemandEmbed.description, color=OnDemandEmbed.color)
                    elif OnDemandEmbed.description == 'None':
                        OnDemandEmbed.example_embed = discord.Embed(title=OnDemandEmbed.title, color=OnDemandEmbed.color)
                        OnDemandEmbed.user_embed = discord.Embed(title=OnDemandEmbed.title, color=OnDemandEmbed.color)

                    OnDemandEmbed.example_embed.set_author(name='Type Done to finish or Cancel to exit', icon_url=ctx.author.avatar_url)
                ...
                prompt = await ctx.send(embed=discord.Embed(description=ctx.author.mention + 'Enter a Title or None', color=OnDemandEmbed.color))
                title = await self.bot.wait_for('message', timeout=60.0, check=check)
                OnDemandEmbed.title = title.content
                await title.delete()
                await prompt.delete()

                prompt = await ctx.send(embed=discord.Embed(description=ctx.author.mention + ' Enter a Description or None', color=OnDemandEmbed.color))
                ...
                while working:
                    ...
                    if input_num.content == '4':
                        ...
                        await sent_example_embed.delete()
                        await input_num.delete()
                        cancel_msg = await ctx.send(embed=discord.Embed(description=ctx.author.mention + ', Operation cancelled', color=OnDemandEmbed.color))
                        await cancel_msg.delete(delay=10)
                        working = False
                    else:
                        await input_num.delete()
                        cancel_msg = ctx.send(embed=discord.Embed(description=ctx.author.mention + ' Valid input please', color=OnDemandEmbed.color))
