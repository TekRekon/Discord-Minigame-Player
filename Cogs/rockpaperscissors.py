...
class rockpaperscissors(commands.Cog):
    ...
    async def rps(self, ctx):
        ...
        await me.send(f"rock paper scissors game initiated")

        if reaction.emoji == '📲':

            p2 = user

            embed.set_author(name=f'Rock Paper Scissors (2 player)', icon_url='https://cdn.discordapp.com/attachments/488700267060133889/808744030618779658/b680a062246147.5a8a773c6932d.gif')
            ...
            while working:
                ...
                embed.description = f'{message1.author.mention} object received, waiting on your opponent: DM me `rock`, `paper`, or `scissors`'
                await sent_embed.edit(embed=embed)
                ...
                if message1.content == message2.content:
                    embed.description = f'{p1.mention} and {p2.mention} tied using {message1.content}. DM me `rock`, `paper`, or `scissors`'
                    await sent_embed.edit(embed=embed)
                else:
        ...
        if reaction.emoji == '🤖':
            ...
            embed.set_author(name=f'Rock Paper Scissors (AI)', icon_url='https://cdn.discordapp.com/attachments/488700267060133889/808744030618779658/b680a062246147.5a8a773c6932d.gif')
            embed.description = f'{p2.mention}: DM me `rock`, `paper`, or `scissors`'
            await sent_embed.edit(embed=embed)
            ...
            while working:
                ...
                if message1.content == message2:
                    embed.description = f'{p1.mention} and {p2.mention} tied using {message2}. DM me `rock`, `paper`, or `scissors`'
                    await sent_embed.edit(embed=embed)
                else:
                    DatabaseTools.addPlayer(message1.author.id, message1.author.name, 'rps')
                    ...
                    if 'rock' in x and 'paper' in x:
                        if message1.content == 'paper':
                            embed.description = f'{message1.author.mention} won against {p2.mention} using `{message1.content}`'
                            DatabaseTools.editPlayerScore(message1.author.id, True, 'rps')
                            DatabaseTools.editPlayerScore(p2.id, False, 'rps')
                            await sent_embed.edit(embed=embed)
                        else:
                            embed.description = f'{p2.mention} won against {message1.author.mention} using `{message2}`'
