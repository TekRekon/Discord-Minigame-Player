    ...
    async def on_message(self, message):
        if message.channel == AutoFormatNewGit.botUpdatesChannel:
            if message.author != self.bot.user:
                ...
                if old_embed.title is not None and old_embed.description is not None:
                    AutoFormatNewGit.git_embed = discord.Embed(title=old_embed.title, description=old_embed.description,
                                                               color=0xff0000)
                elif old_embed.description is None:
                    AutoFormatNewGit.git_embed = discord.Embed(title=old_embed.title, color=0xff0000)
                else:
                    AutoFormatNewGit.git_embed = discord.Embed(description=old_embed.description, color=0xff0000)
                for x in old_embed.fields:
                    AutoFormatNewGit.git_embed.add_field(name=x.name, value=x.value, inline=x.inline)
                if old_embed.author.name is not None and old_embed.author.icon_url is not None:
                    AutoFormatNewGit.git_embed.set_author(name=old_embed.author.name,
                                                          icon_url=old_embed.author.icon_url)
                await message.channel.send(AutoFormatNewGit.botNotifsRole.mention)
                await message.channel.send(embed=AutoFormatNewGit.git_embed)
